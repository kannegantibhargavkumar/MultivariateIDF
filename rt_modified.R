### this package works with vinecopula association
library(VineCopula)
library(copula)

genEmpKenFun <- function(copula, sample=NULL) {
  if(is.null(sample)) 
    sample <- BiCopSim(1e6,copula)
  if(missing(copula)) {
...
criticalPair <- function(copula, cl, u, ind, tol=sqrt(.Machine$double.eps)) {
  ...
  optimFun <- function(x, u, ind) {
    pair <- cbind(x,x)
    pair[,ind] <- u
    return(abs(BiCopCDF(pair[,1],pair[,2],copula)-cl))
  }
...
criticalTriple <- function(copula, cl, u, ind, tol=sqrt(.Machine$double.eps)) {
  ...
  optimFun <- function(x, u, ind) {
    ...
    x[,ind[1]] <- u[1]
    x[,ind[2]] <- u[2]
    return(abs(BiCopCDF(x,copula)-cl))
  }
